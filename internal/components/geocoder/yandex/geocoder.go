...
func Geocoder(apiKey string, baseURLs ...string) geocoder.Geocoder {
	return geocoder.HTTPGeocoder {
		EndpointBuilder: baseURL(getURL(apiKey, baseURLs...)),
		ResponseParserFactory: func() geocoder.ResponseParser {
...
func (b baseURL) ReverseGeocodeURL(l geocoder.Location) string {
	return string(b) + fmt.Sprintf("sco=latlong&geocode=%f,%f", l.Lat, l.Lng)
}

func (r *geocodeResponse) Location() (*geocoder.Location, error) {
...
func (r *geocodeResponse) Address() (*geocoder.Address, error) {
	if r.Response.GeoObjectCollection.MetaDataProperty.GeocoderResponseMetaData.Found == "0" {
		return nil, nil
	}
...
func parseYandexResult(r *yandexFeatureMember) *geocoder.Address {
